...
function comment_delete(id) {
  ...
  for (let i = 0; i < addrArr.length; i++) {
    ...
    if (getPageAddr.indexOf(addrArr[i]) != -1) {
      if (i == 3) {
        //saleDetail(삽니다, 무료나눔)은 방식이 달라 재확인 필요
        request.setAttribute("action", "/comment/commentDelete");
        break;
      }
      // saleDetail 이외 getaddr에서 맞는 값 리턴
...
function comment_edit(id) {
  ...
  if (ckCom == "") alert("내용을 입력하세요");
  else {
    // 수정할 텍스트를 받아 해당하는 api에 get방식으로 전달해줌
    for (let i = 0; i < addrArr.length; i++) {
      if (getPageAddr.indexOf(addrArr[i]) != -1) {
        if (i == 3) {
        ...
        request.setAttribute(
          "action",
          "/comment/commentEdit_" + getAddr[i] + "?Com"
        );
        break;
      }
    }
