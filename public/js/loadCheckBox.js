...
function loadCheckCate() {
  ...
  let strSplit = getCate.split(","); // 카테고리 목록을 ',' 단위로 나눔 DB 속성에 카테고리는 ,로 한줄로 표현되어있다
  ...
  for (let i = 1; i < 11; i++) {
    ...
    for (let j = 0; j < 3; j++) {
      // 카테고리 값과 데이터베이스에서 받아온 카테고리 값이 일치하면 해당 체크박스를 체크한다
      if (strSplit[j] == cateVal[i]) cateArr[i].checked = true;
    }
  }
}
...
function loadCheckState() {
  ...
  for (let i = 1; i <= 5; i++) {
    ...
    for (let j = 0; j < strSplitLen; j++) {
      if (strSplit[j] == stateVal[i]) stateArr[i].checked = true;
    }
  }
}
...
function loadtradeRoot() {
  ...
  for (let i = 1; i <= 3; i++) {
    ...
    if (getMethod == tradeVal[i]) tradeArr[i].selected = true;
  }
}
