...
router.get("/", async (req, res, next) => {
  try {
    ...
    if (req.user) {
      ...
      const [sellBooksIndex] = await Promise.all([
        // index에 무료나눔
        Book.findAll({
          where: {
            SoldId: null,
      ...
      const [wannabuyBooksIndex] = await Promise.all([
        // index에 무료나눔
        Book.findAll({
          where: {
            SoldId: null,
...
router.post("/book/:id/comment", isLoggedIn, async (req, res, next) => {
  try {
    ...
    return res.send(
      `<script type="text/javascript">location.href="/book/${post.BookId}";</script>`
    );
  } catch (error) {
    console.error(error);
    next(error);
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      res.send(
        `<script type="text/javascript">alert("책 등록 완료"); location.href="/book/${book.id}";</script>`
      ); // 등록 하고 자기가 등록한 책 화면 띄우게 하기
    } catch (error) {
      console.error(error);
      next(error);
    }
...
router.post("/like", isLoggedIn, async (req, res, next) => {
  try {
    ...
    const isheliked = await Who.findOne({
      where: { thisbook: bookId, liked: req.user.id },
    });
    if (String(req.user.id) === String(owner)) {
      return res.send(
        `<script type="text/javascript">alert("자신의 물건에는 할 수 없습니다."); location.href="/book/${bookId}";</script>`
      );
    } else if (isheliked) {
      const FindBook = await Book.findOne({
        where: { id: bookId, OwnerId: owner },
      });
      const add = FindBook.likecount - 1;
      await Who.destroy({
        where: { thisbook: FindBook.id, liked: req.user.id },
      });
      await Book.update(
        {
          likecount: add,
      ...
      return res.send(
        `<script type="text/javascript">alert("찜 해제됐습니다!"); location.href="/book/${bookId}";</script>`
      );
    } else {
      const FindBook = await Book.findOne({
        where: { id: bookId, OwnerId: owner },
      ...
      return res.send(
        `<script type="text/javascript">alert("찜 했습니다!"); location.href = "/book/${bookId}"; </script>`
      );
    }
  } catch (error) {
    console.error(error);
